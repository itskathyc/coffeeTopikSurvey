...
exports.handler = async function (event, context) {
    ...
    const NETFLIY_SITE_ID = process.env.NETFLIY_SITE_ID;
    const AUTH_TOKEN = process.env.NETLIFY_AUTH_TOKEN;

    if (!NETFLIY_SITE_ID || !AUTH_TOKEN) {
        return {
            statusCode: 500,
    ...
    try {
        const response = await fetch(`https://api.netlify.com/api/v1/sites/${NETFLIY_SITE_ID}/submissions`, {
